...
1 - Introduction:
	...
	This set of classes is designed to show the reliability of java RMI.
...
2 - Use - General:
	...
	To start the server, at the command prompt:
		...
		b) Run the server by typing the following command.
			
		java -Djava.security.policy=../openall.policy server/RMIServer
	...
	To start the client, at the command prompt:
		...
		java -Djava.security.policy=../openall.policy client/RMIClient hostname  num_messages
		
2.1 - Easy use with windows batch files
	
	To compile the source files in command prompt run compile.bat
	To run the server type run-server
	To run the client type run-client host_name num_messages
